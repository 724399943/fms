...
$(".city").each(function() {
	if ( $(this).next('.county').length <= 0 ) {
		var string = [];
		string.push('&nbsp;&nbsp;<select class="county select-date" name="area" style="display:inline-block;">');
		string.push('<option value="-1">--未选择--</option>');
		string.push('</select>');
		$(this).after(string.join(''));
	}
...
function fullData(data, element, address_id) {
	...
	for ( var i in data ) {
		if(address_id == data[i].id ) {
			string.push('<option value="' + data[i].id + '" selected="selected">' + data[i].region_name + '</option>');
		} else {
