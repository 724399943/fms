...
<div class="headCont JproductSearch" >
    <div class="categoryList">
        <div class="isearch">
        <form method="post" action="<?php echo U('Search/search');?>" id="serchform">
            <em class="sea sea2"></em>
            <input type="hidden" name="type" value="product" />
            <input type="search" name="keyword" placeholder="搜索商品" id="Jinput" value="<?php echo ($keyword); ?>" style="border: none">
            <em class="close" id="JclearInput"></em>
        </form>
        </div>
        ...
        <div class="iseaResult">
          <?php if(empty($search)): ?><!-- 无结果 -->
            <div class="noResult">
                <img src="/Static/Public/Wap/fms/images/icon10.png">
                <p>暂无数据~</p>
            </div>
            <a href="<?php echo U('Goods/goods');?>" class="all">查看全部产品</a>

            <?php else: ?>
            
            <!-- 有结果 -->
            <ul class="indgoc god">
            <?php if(is_array($search)): $i = 0; $__LIST__ = $search;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
            ...
            <a href="<?php echo U('Goods/goods');?>" class="all">查看全部产品</a><?php endif; ?>
