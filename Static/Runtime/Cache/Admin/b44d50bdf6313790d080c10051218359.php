...
<head>
    ...
    <title><?php echo ($return['cat_name']); ?></title>
...
<?php if($vo['module_id'] != '7'): ?><li>
    ...
    <div class="pageheader">
        <h1 class="pagetitle"><?php echo ($return['cat_name']); ?></h1>
    </div>
    <div class="contentwrapper">
        <form class="order-list" method="GET" action="<?php echo U('Article/articleList');?>">
            <p>
                <input type="button" value="删除" class="Kdelete" />
                &nbsp;&nbsp;&nbsp;
                分类：
                <select name="cat_id">
                <option value="-1" <?php if($return['cat_id'] == '-1'): ?>selected<?php endif; ?>>所有分类</option>
                    <?php $selected = ''; function showTree ($data,$pid) { foreach ($data as $key => $val){ $selected = ( $val['id'] == $pid) ? 'selected' : ''; if ($val['depth'] >= 1) { $string = str_repeat('│ ',$val['depth'] - 1); echo '<option value="'.$val['id'].'" '.$selected.'>'.$string .'├ ' .$val['cat_name'].'</option>'; } else { echo '<option value="'.$val['id'].'" '.$selected.' >' .$val['cat_name'].$val['depth'].'</option>'; } if (!empty($val['childCategory'])) { showTree($val['childCategory'],$pid); } } } showTree($categoryList,$return['cat_id']); ?>
                </select>
                &nbsp;&nbsp;&nbsp;&nbsp;
                标题：
                <input type="text" class="input-text" style="width:250px" placeholder="文章标题" name="title" value="<?php echo ($return['title']); ?>" >
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                ...
                <?php else: ?>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php if(empty($$vo['image'])): ?>无<?php else: echo ($vo['image']); endif; ?></td>
                            <td><?php if($vo['flag'] == 1): ?>启用<?php else: ?>禁用<?php endif; ?></td>
                            <td><?php echo (time_format($vo['add_time'])); ?></td>
                          
                            <td>
                                <?php if(checkActionAuth('Article-editArticle')): ?><a class="stdbtn btn_lime" href="<?php echo U('Article/editArticle', array('id'=>$vo['id']));?>">编辑</a>
                                <?php else: ?>
                                    无权限访问<?php endif; ?>
                                <?php if(checkActionAuth('Article-delArticle')): ?><a class="stdbtn btn_lime" href="<?php echo U('Article/delArticle', array('ids'=>$vo['id']));?>">删除</a>
                                <?php else: ?>
                                    无权限访问<?php endif; ?>
                            </td>
