...
<?php if($vo['module_id'] != '7'): ?><li>
    ...
    <div class="contentwrapper">
        <form class="order-list" method="GET" action="<?php echo U('Field/fieldList');?>">
            <p>
                ...
                <select name="tree_id">
                    ...
                    <?php if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($return['tree_id'] == $vo['id']): ?>selected<?php endif; ?>><?php echo ($vo['cat_name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
                <a href="<?php echo U('Field/addField',array('firstCatId'=>$firstCatId));?>" class="btn btn_link" style="float: right;margin-right: 10px;">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                ...
                <?php else: ?>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($vo['input_type']); ?></td>
                            <td><?php echo ($vo['tree_name']); ?></td>
                            <?php if($vo['is_valid'] == '1'): ?><td class="Jvalid" data-id="<?php echo ($vo['id']); ?>-2">
                            ...
                            <td>
                                <?php if(checkActionAuth('Field-editField')): ?><a class="stdbtn btn_lime" href="<?php echo U('Field/editField', array('id'=>$vo['id']));?>">编辑</a>
                                <?php else: ?>
                                    无权限访问<?php endif; ?>
                                <?php if(checkActionAuth('Field-delField')): ?><a class="stdbtn btn_lime" href="<?php echo U('Field/delField', array('ids'=>$vo['id']));?>">删除</a>
                                <?php else: ?>
                                    无权限访问<?php endif; ?>
                            </td>
...
$('.Jvalid').click(function(){
        ...
        $.ajax({
            url: "<?php echo U('Field/changeFieldStatus');?>",
            type: 'post',
            dataType: 'json',
...
$('.Jdelete').click(function(){
    ...
    var jumpUrl = "<?php echo U('Field/delField');?>?ids=" + ids;
    window.location.href = jumpUrl;
})
</script>
