...
<?php if($vo['module_id'] != '7'): ?><li>
    ...
    <div id="contentwrapper" class="contentwrapper">
        <form class="stdform stdform2" action="<?php echo U('Article/addArticle');?>" method="post" id="question-form">
        <div class="contenttitle2">
                ...
                <span class="field">
                    <select name="cat_id">
                        <option value="">所有分类</option>
                            <?php function showTree($list,$return){ foreach( $list as $key=>$value ){ if( $value['depth'] >= 1){ echo '<option value="'.$value['id'].'">'.str_repeat('&nbsp;&nbsp;|',$value['depth']-1).'&nbsp;├ ' . $value['cat_name'].'</option>'; }else{ echo '<option value="'.$value['id'].'" >'.$value['cat_name'].'</option>'; } if( !empty($value['childCategory']) ){ showTree($value['childCategory'],$return); } } } showTree($categoryList,$return); ?>
                    </select>
                </span>
            </div>
            ...
            <?php if(is_array($extendFieldList)): $firstKey = 0; $__LIST__ = $extendFieldList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($firstKey % 2 );++$firstKey;?><input type="hidden" name="module_attr_id[]" value="<?php echo ($vo['id']); ?>" />
                <div class="line-dete">
                <label><?php echo ($vo['type_name']); if($vo['is_valid'] == 1): ?><em>*</em><?php endif; ?></label>
                    <span class="field">
                        <?php switch($vo['input_type']): case "text": ?><input type="text" name="field[attr_<?php echo ($vo['id']); ?>]" /><?php break;?>
                            <?php case "textarea": ?><textarea name="field[attr_<?php echo ($vo['id']); ?>]"><?php echo ($vo['ext_value']); ?></textarea><?php break;?>
                            <?php case "checkbox": if(is_array($vo['attr_value'])): $i = 0; $__LIST__ = $vo['attr_value'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><input type="checkbox" name="field[attr_<?php echo ($vo['id']); ?>][]" value="<?php echo ($item); ?>" /><?php echo ($item); ?>&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; break;?>
                            <?php case "radio": if(is_array($vo['attr_value'])): $k = 0; $__LIST__ = $vo['attr_value'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($k % 2 );++$k;?><input type="radio" name="field[attr_<?php echo ($vo['id']); ?>]" value="<?php echo ($item); ?>" /><?php echo ($item); ?>&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; break;?>
                            <?php case "select": ?><select name="field[attr_<?php echo ($vo['id']); ?>]">
                                    <?php if(is_array($vo['attr_value'])): $i = 0; $__LIST__ = $vo['attr_value'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><option value="<?php echo ($item); ?>"><?php echo ($k); echo ($item); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                </select><?php break;?>
                            <?php case "img": ?><input type="text" name="field[attr_<?php echo ($vo['id']); ?>]" id="imgs2" class="smallinput" readonly="true"/>
                                <input type="file" name="up-pic" id="up-pic2"><?php break;?>
                            <?php case "attachment": ?><input type="text" name="field[attr_<?php echo ($vo['id']); ?>]" id="imgs3" class="smallinput" readonly="true"/>
            ...
            <div class="line-dete">
                <label>TAG标签：</label>
                <span class="field">
                    <input type="text" class="smallinput" placeholder="<?php echo ($tags); ?>" value="<?php echo ($tags); ?>" name="tags">
                </span>
            </div>
            ...
            <div class="line-dete">
                <label>Meta关键字：</label>
                <span class="field">
                    <textarea name="meta_keyword" id="" cols="30" rows="10" style="margin: 0px; height: 62px; width: 629px;"><?php echo ($meta_keyword); ?></textarea>
                </span>
            </div>
            ...
            <div class="line-dete">
                <label>Meta描述：</label>
                <span class="field">
                    <textarea name="meta_description" id="" cols="30" rows="10" style="margin: 0px; height: 68px; width: 678px;"><?php echo ($meta_description); ?></textarea>
                </span>
            </div>
