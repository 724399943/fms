            ...
            <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablequick">
                ...
                <?php else: ?>
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            <td><input type="checkbox" class="ids" value="<?php echo ($vo['id']); ?>" /><?php echo ($vo['id']); ?></td>
                            <td><?php echo ($vo["username"]); ?></td>
                            <td><?php echo ($vo['ip']); ?></td>
                            <td><?php echo ($vo['timeline']); ?></td>
                            <td><?php echo ($vo['content']); ?></td>
                            <td>
                                <?php if($vo['success'] == '1'): ?><em class="btn_success">成功</em>
                                <?php else: ?>
                                    <em class="btn_fail">失败</em><?php endif; ?>
                            </td>
                            <td>
                                <?php if(checkActionAuth('Log-delLog')): ?><a class="stdbtn btn_lime" href="<?php echo U('Log/delLog', array('ids'=>$vo['id']));?>">删除</a>
                                <?php else: ?>
                                    无权限访问<?php endif; ?>
                            </td>
...
$('.Jdelete').click(function(){
    ...
    var jumpUrl = "<?php echo U('Log/delLog');?>?ids=" + ids;
